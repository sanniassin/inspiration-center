...
function parseBlogArticles(url) {
	return new Promise((resolve, reject) => {
...
module.exports.populateData = (config, parser) => {
	return new Promise((resolve, reject) => {
		...
		let useCasePromises = config.useCases.map(item => {
			...
			let blogArticlePromises = Promise.all(blogArticles.map(blogArticleUrl => parseBlogArticles(blogArticleUrl)))
		...
		let gettingStartedPromises = config.gettingStarted.map(item => {
			...
			return videoPromises
				.then(results => {
					item.videos = prepareVideos(results)
					return item
				})
		})
