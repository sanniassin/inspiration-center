...
// todo // not working with rejects. gulp doesn't emit errors
		...
		fs.readFile(path.join(cwd, 'schemas', `${schemaName}.json`), (err, data) => {
			...
			if (valid) {
				if (tv4.missing && tv4.missing.length) {
					throw new Error(`Missing schemas: ${JSON.stringify(tv4.missing)}`)
				} else {
					resolve()
				}
			} else {
				throw new Error(`Validating file ${filePath}\nProperty: ${tv4.error.dataPath}, Message: ${tv4.error.message}`)
			}
		})
	})
}