...
module.exports.populateData = (config, parser) => {
	return new Promise((resolve, reject) => {
		...
		let gettingStartedPromises = config.gettingStarted.map(item => {
			...
			let interfaceExplained = item.interfaceExplained || []
			let interfaceExplainedPromises = Promise.all(interfaceExplained.map(item => composeId(item, 'file', '.pdf')))

			return Promise.all([videoPromises, helpTopicsPromises, howToPromises, interfaceExplainedPromises])
				.then(results => {
					item.videos = prepareVideos(results[0])
					item.helpTopics = results[1]
					item.howTo = results[2]
					item.interfaceExplained = results[3]
					return item
				})
		})
